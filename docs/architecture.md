...
### Fase 3 - UX:
- Rich library para UI avanzada
- Progress bars
- SQLite para memoria persistente
- Auto-completado de comandos

### Fase 4 - Avanzado:
- Multi-threading para herramientas paralelas
- WebSocket API para dashboard
...
- `logs/session_*.json` - Acciones de usuario
- `memory/session_*_memory.json` - Conversaciones
- `cai_env_sexo/logs/*.jsonl` - Logs de CAI framework

### Troubleshooting:

```python
# Modo debug en main.py:
import logging
logging.basicConfig(level=logging.DEBUG)

# Ver herramientas registradas:
tool_manager.print_tools_table()

# Ver sesión actual:
print(controller.get_session_summary())
```
